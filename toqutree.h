...
private:
   ...
   */
    ...
    void r(PNG &im, toqutree::Node* node);
   int prune(double tol, toqutree::Node* node);
   int sizehelper(Node* n);
   pair<int,int> buildTree(PNG * im, pair<int, int> ul, pair<int, int> lr, stats* s ,int dim);
   double entP(PNG * im, pair<int, int> ul, pair<int, int> lr, pair<int, int> locul, pair<int,int> loclr, stats* s);
   vector<int> addHist(vector<vector<int>> s);
